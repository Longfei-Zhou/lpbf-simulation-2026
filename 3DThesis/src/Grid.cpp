...
double Grid::Calc_Solidification_time(const double t, const Simdat& sim, const int p) {
	...
	// Iterate to the requested solidification temperature.
	while (runFlag) {
		m = (T2 - T1) / (t2 - t1);
...
vector<vector<double>> Grid::Calc_Solidficiaton_Primary(const double t, const Nodes& nodes, const int p) {
	...
	**************************************************************************************************/
	...
	for (size_t iter = 0; iter < nodes.size; iter++) {
		...
		Laplace += dT_seg * (dpx * dpx + dpy * dpy + dpz * dpz + ddpx + ddpy + ddpz);
		// A zero-width interval represents the instantaneous source contribution.
		dT_t += (nodes.dtau[iter]== 0) ? phi : 0;
	}
