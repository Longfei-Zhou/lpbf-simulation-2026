...
void	Init::ReadSimParams(Simdat& sim) {
	...
	Init::FileRead_Beams(sim.beams, sim.files.beam, sim.print);
	Init::FileRead_Material(sim.material, sim.files.material, sim.print); Util::Calc_NonD_dt(sim.beams, sim.material);
	Init::FileRead_Paths(sim.paths, sim.files.path, sim.print); Util::Calc_AllScansEndTime(sim);
	
	Init::FileRead_Mode(sim, sim.files.mode);

	Init::FileRead_Domain(sim.domain, sim.files.domain, sim.print); Util::Calc_ScanBounds(sim.domain, sim.paths); Init::SetDomainParams(sim.domain);
	Init::FileRead_Output(sim.output, sim.files.output, sim.print);
	Init::FileRead_Settings(sim.settings, sim.files.settings, sim.print); Util::Calc_RMax(sim);
...
void	Init::FileRead_Path(vector<path_seg>& path, const string& file, const bool print) {	
	// Path coordinates are converted from millimetres to metres.
	double convert = 1e-3;
...
void	Init::FileRead_Domain(Domain& domain, const string& file, const bool print) {
	...
	// Custom point files define an explicit, possibly non-rectangular domain.
...
void	Init::SetDomainParams(Domain& domain) {
	...
	// A custom point file already defines every requested coordinate.
